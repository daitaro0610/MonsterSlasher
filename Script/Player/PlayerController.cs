...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField]
    private VirtualCamera m_CounterCamera;
    private CounterCameraAnimation m_CounterCameraAnimation;
    ...
    private void Awake()
    {
        ...
        m_PlayerVoice = new();
        m_KatanaSe = new();
        m_CounterCameraAnimation = new(m_CounterCamera);
    ...
    {
        //カウンター用のカウンターアニメーション
        m_CounterCameraAnimation.ApplyCameraMove();
        m_Animation.Counter();
    ...
    private void AnimAvoidance(int index)
    {
        ...
        switch (index)
        {
            case 0: //正面に回避
                ...
                EffectManager.instance.PlayPosition(EffectName.AvoidanceSmoke, transform.position);

                break;
            case 1: //右方向に回避
                m_Movement.Avoidance(transform.right);
                {
                    Vector3 angle = transform.eulerAngles + Vector3.up * 90;
                    EffectManager.instance.PlayPosition(EffectName.AvoidanceSmoke, transform.position, rotation: angle);
                }
    ...
    private void SheathingSwordEnd()
    {
        m_BaseState = PlayerBaseState.Normal;
    }
